...
class MedicControllerTest {
    ...
    void deleteMedicSuccessfully() {
        String crm = "12345";
        doNothing().when(medicService).deleteMedic(crm);

        ResponseEntity<String> response = medicController.deleteMedic(crm);

        assertEquals("Medic deleted successfully", response.getBody());
        assertEquals(200, response.getStatusCodeValue());
        verify(medicService, times(1)).deleteMedic(crm);
    }
